...
const CampaignDetails = () => {
  ...
  const { donate, getDonations, contract, address, getCampaignById } =
    useStateContext();
  ...
  const fetchCampaign = async () => {
    setIsLoading(true);
    const data = await getCampaignById(idUrl);
    setCampaign(data);
    setIsLoading(false);
  };
  ...
  const onDonateSuccess = () => {
    if (window.flutter_inappwebview) {
      window.flutter_inappwebview
        .callHandler("SuperAppChannel", "Donate Success")
        .then((result) => {
          console.log(result);
        })
        .catch((error) => {
                    ...
                    donators.map((item, index) => (
                      ...
                      >
                        <div className="flex flex-1 w-[60%]">
                          <p className="font-epilogue line-clamp-2 font-normal text-[16px] text-[#b2b3bd] leading-[26px]">
